...
test('Testing post object endPoint.', async () => {
    await request(app).post('/object').send(object).expect(201)
})

test('Testing get object value by key name.', async () => {
    await request(app).get(`/object/${object.key}`)
    .expect(200)
    .expect({value: object.value})
})

test('Testing get lastest object value by key name.', async () => {
    await request(app).post('/object').send(object2)
    await request(app).get(`/object/${object.key}`)
    .expect(200)
...
test('Testing get object value by key name while filtering by timestamp', async () => {
    const response2 = await request(app).post('/object').send(object2)
    const timestamp2 = response2.body['timestamp'] + 2
    console.log('timestamp2 === ', timestamp2)
    console.log('response2 body === ', response2.body)

    const response3 = await request(app).post('/object').send(object3)
    const timestamp3 = response3.body['timestamp'] + 2
    await request(app).get(`/object/${object.key}?timestamp=${timestamp2}`)
    .expect(200)
    .expect({
        value: response2.body.value
    })
    await request(app).get(`/object/${object.key}?timestamp=${timestamp3}`)
    .expect(200)
    .expect({
